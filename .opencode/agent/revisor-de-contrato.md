...
- `contrato-api.md` — seções 5 (rotas, com método, caminho e campos) e 6 (códigos de retorno, com status e "onde aparece");
...
3. Compare cada rota da API com o contrato: rota que não existe no contrato é divergência — inclusive no `MODO_TESTE`, pois nenhuma rota fora do contrato entra no código, nem como costura de teste.
...
1. [ROTA FORA DO CONTRATO] `POST /_teste/…` — não existe na seção 5; registrada em api/src/…:linha.
